...
class TicketAPI(Resource):
    ...
    def post(self, user_id=""):
        ...
        try:
            ...
            details["created_at"] = int(time.time())
        ...
        try:
            ...
            if role == "student":
                if user_id == ticket.created_by:
                    ...
                    ticket_vote = VoteTable.query.filter_by(
                        ticket_id=ticket_id, voter_id=user_id
                    ).first()
                    if ticket_vote:
            ...
            if role == "support":
                ...
                if ticket_utils.is_blank(sol):
                    ...
                    try:
                        _from = user.email
                        ticket_votes = VoteTable.query.filter_by(
                            ticket_id=ticket_id
                        ).all()
                        users_voted = [
                            ticket_vote.voter_id for ticket_vote in ticket_votes
                        ]
                        users_voted.append(ticket.created_by)
                        users_email_name = []
                        for user_id_ in users_voted:
                            user_ = User.query.filter_by(user_id=user_id_).first()
...
class AllTicketsUserAPI(Resource):
    ...
    def get(self, user_id=""):
        ...
        if role == "student":
            ...
            upvoted_ticket_ids = VoteTable.query.filter_by(user_id=user.user_id).all()
